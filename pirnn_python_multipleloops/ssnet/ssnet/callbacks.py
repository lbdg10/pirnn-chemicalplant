...
class PostEpochTrainingData():
    deltaiss_residuals_loop: List[float | torch.Tensor]
    enforce_deltaiss_loop: bool
    epoch: int
    train_loss: float
    train_metric: float
    val_metric: float
...
class CallbacksWrapper(PostEpochCallback):
    ...
    def on_training_end(self, data: PostEpochCallback, net: StateSpaceNN) -> dict:
        if len(self.callbacks) == 0:
            return
...
class LoggingCallback(PostEpochCallback):
    ...
    def on_training_start(self, net: StateSpaceNN):
        ...
        self.log['deltaiss_residuals_loop'] = []
        self.log['epoch_time'] = []
    ...
    def on_epoch_end(self, data: PostEpochTrainingData, net: StateSpaceNN) -> bool:
        ...
        self.log['deltaiss_residuals_loop'].append(data.deltaiss_residuals_loop)
        ...
        if self.tensorboard is not None:
            ...
            self.tensorboard.add_scalars(main_tag='Residuals/deltaISSloop', tag_scalar_dict=self._list2dict(data.deltaiss_residuals_loop), global_step=data.epoch)
    ...
    def on_training_end(self, data: PostEpochTrainingData, net: StateSpaceNN) -> dict:
        ...
        with torch.no_grad():
            ...
            deltaiss_residuals_loop = [nu.detach().item() for nu in net.deltaiss_residuals_loop()]
            is_deltaiss_loop = all([nu < 0 for nu in deltaiss_residuals_loop])
        ...
        return {'final_train_metric': self.log['train_metric'][-1], 'final_val_metric': self.log['val_metric'][-1],
                ...
                'final_deltaiss_residuals_loop':deltaiss_residuals_loop, 'is_deltaiss_loop': is_deltaiss_loop}

class SigIntCallback(PostEpochCallback):
...
class EarlyStoppingCallback(PostEpochCallback):
    ...
    def on_epoch_end(self, data: PostEpochTrainingData, net: StateSpaceNN) -> bool:
        ...
        deltaiss_loop_satisfied = not data.enforce_deltaiss_loop or all([r < 0.0 for r in data.deltaiss_residuals_loop])

        if data.val_metric < self._overall_best_metric:
            self._overall_best_epoch = data.epoch
            self._overall_best_metric = data.val_metric

        if data.val_metric < self.best_metric and iss_satisfied and deltaiss_satisfied and deltaiss_loop_satisfied:
        ...
        if data.enforce_iss or data.enforce_deltaiss or data.enforce_deltaiss_loop:
        ...
        if self.tensorboard is not None:
            ...
            if data.enforce_iss or data.enforce_deltaiss and self.best_epoch: #or data.enforce_deltaiss_loop 
...
class TargetMetricCallback(PostEpochCallback):
    ...
    def on_epoch_end(self, data: PostEpochTrainingData, net: StateSpaceNN) -> bool:
        ...
        self.target_fired = data.val_metric <= self.target and \
                                ...
                                data.enforce_deltaiss and all([r < 0.0 for r in data.deltaiss_residuals]) or
                                data.enforce_deltaiss_loop and all([r < 0.0 for r in data.deltaiss_residuals_loop]))
...
class PerformanceTestingCallback(PostEpochCallback):
    ...
    # def _compute_FIT_min(self, y_hat: torch.Tensor, y_test: torch.Tensor):
    #     sim_err = np.zeros((y_test.shape[2]))
    #     y_ampl = np.zeros((y_test.shape[2]))
    #     err_dev = np.zeros((y_test.shape[2]))
    #     FIT = np.zeros((y_test.shape[2]))
    #     mean = np.zeros((y_test.shape[2]))
    #     for i in range(0, y_test.shape[2]):
    #         mean[i] = y_test[0][:][i].mean()
    #         sim_err[i] = torch.linalg.vector_norm(y_hat[0][:][i] - y_test[0][:][i], ord=2)
    #         y_ampl[i] = torch.linalg.vector_norm(y_test[0][:][i] - mean[i], ord=2)
    #         err_dev[i] = sim_err[i] / y_ampl[i]
    #         FIT[i] = (1 - err_dev[i])*100
    #     fit = min(FIT)
    #     return fit

    # def _compute_FIT_max(self, y_hat: torch.Tensor, y_test: torch.Tensor):
    #     sim_err = np.zeros((y_test.shape[2]))
    #     y_ampl = np.zeros((y_test.shape[2]))
    #     err_dev = np.zeros((y_test.shape[2]))
    #     FIT = np.zeros((y_test.shape[2]))
    ...
    def on_training_end(self, data: PostEpochTrainingData, net: StateSpaceNN) -> dict:
        ...
        fit_index = self._compute_FIT(y_hat, y_test)
        # fit_min = self._compute_FIT_min(y_hat, y_test)
        # fit_max = self._compute_FIT_max(y_hat, y_test)
        ...
        return {'final_test_metric': test_metric, 'FIT': fit_index} # , 'FIT_min': fit_min, 'FIT_max': fit_max
        
class MatlabExportCallback(PostEpochCallback):
    """
