...
from .callbacks import PostEpochCallback, PostEpochTrainingData
...
class StateSpaceNN(nn.Module):
    ...
    def forward(self, u: Tensor, x0: Tensor = None, auto_normalization: bool = False) -> Tuple[Tensor, Tensor]:
        ...
        # Added for physics-based loops in place of preceding lines        
        y2k_to1 = 0.5*torch.randn(un.shape[-2], 4, requires_grad=False) # dimension is (batch size, number of additional input)
        y6k_to1 = 0.5*torch.randn(un.shape[-2], 4, requires_grad=False)
        for t, ut in enumerate(un):
            y, x0, y2k_to1, y6k_to1 = self._recurrence(ut, x0, y2k_to1, y6k_to1)
            state.append(x0)
            output.append(y)
    ...
    def deltaiss_residuals_loop(self) -> List[Tensor]:
        residuals = []
        beta1 = 0.3
        beta2 = beta1
        beta3 = beta1
        ...
        for cell in self.layers:
            if isinstance(cell, StateSpaceRecurrentLayer):
                gamma[i] = cell.compute_gamma()
                i+=1
            else:
                if j != 1:
                    Uo_norm[j] = torch.linalg.norm(cell.weight, ord=np.inf)
                    j+=1
                else:
                    Uo_norm[1] = torch.linalg.norm(cell.weight[0:4,:], ord=np.inf)
                    Uo_norm[2] = torch.linalg.norm(cell.weight[4:8,:], ord=np.inf)
                    j+=2
        #res = (beta1*beta2*beta3*beta4*beta5*beta6)*(gamma[0]*gamma[1]*Uo_norm[0]*Uo_norm[1] / (beta1*beta2*phi1*phi2) + gamma[0]*gamma[1]*gamma[2]*gamma[3]*gamma[4]*gamma[5]*Uo_norm[0]*Uo_norm[2]*Uo_norm[3]*Uo_norm[4]*Uo_norm[5]*Uo_norm[6] / (beta1*beta2*beta3*beta4*beta5*beta6*phi1*phi2*phi3*phi4*phi5*phi6) - 1) # 60 stati
        res = (beta1*beta2*beta3*beta4*beta5*beta6)**2*(gamma[0]*gamma[1]*Uo_norm[0]*Uo_norm[1] / (beta1*beta2*phi1*phi2) + gamma[0]*gamma[1]*gamma[2]*gamma[3]*gamma[4]*gamma[5]*Uo_norm[0]*Uo_norm[2]*Uo_norm[3]*Uo_norm[4]*Uo_norm[5]*Uo_norm[6] / (beta1*beta2*beta3*beta4*beta5*beta6*phi1*phi2*phi3*phi4*phi5*phi6) - 1) # 30 stati
        # res1 = beta1*(gamma[0]*Uo_norm[1]/(beta1*phi2) - 1)
        # res2 = beta1*(gamma[1]*Uo_norm[0]/(beta2*phi1) + gamma[1]*Uo_norm[2]/(beta2*phi3) - 1)
        # res3 = beta1*(gamma[2]*Uo_norm[3]/(beta3*phi4) - 1)
        # res4 = beta1*(gamma[3]*Uo_norm[4]/(beta4*phi5) - 1)
        # res5 = beta1*(gamma[4]*Uo_norm[5]/(beta5*phi6) - 1)
        # res6 = beta1*(gamma[5]*Uo_norm[6]/(beta6*phi1) - 1)
        if isinstance(res, Tuple):
            residuals += list(res)
        elif isinstance(res, torch.Tensor):
            residuals += [res]
        ...
        #     residuals += list(res1) + list(res2) + list(res3) + list(res4) + list(res5) + list(res6)
        # elif isinstance(res1, torch.Tensor):
    ...
    def fit(
        ...
        epochs: int = 1e4,
        washout: int = 0,
        callbacks: PostEpochCallback = None) -> dict:
        ...
        for epoch in train_msg:
            ...
            callbacks.on_epoch_start(self, epoch=epoch)
            ...
            for batch in train_loader:
                ...
                if iss_regularizer is not None:
                    loss = loss + iss_regularizer(self.iss_residuals())
                if deltaiss_regularizer is not None:
                    loss = loss + deltaiss_regularizer(self.deltaiss_residuals())
                    #loss = loss + 1/(0.3**3)*deltaiss_regularizer(self.deltaiss_residuals_loop()) # 60 stati condizione somma valori
                    #loss = loss + 100/(0.3**6)*deltaiss_regularizer(self.deltaiss_residuals_loop()) # 30 stati condizione somma valori
                    #loss = loss + deltaiss_regularizer(self.deltaiss_residuals_loop()) # 30 stati condizione Gersgorin
                    loss = loss + 150/(0.3**6)*deltaiss_regularizer(self.deltaiss_residuals_loop()) # 18 stati condizione somma valori
            ...
            with torch.no_grad() and self.evaluating():
                ...
                deltaiss_residuals_loop = [nu.detach().item() for nu in self.deltaiss_residuals_loop()]
                val_metric_avg = 0.0
            ...
            epoch_data = PostEpochTrainingData(
                deltaiss_residuals_loop=deltaiss_residuals_loop,
                enforce_deltaiss_loop=deltaiss_regularizer is not None,
                epoch=epoch,
            ...
            if callbacks.on_epoch_end(epoch_data, self):
                print(f"Training procedure stopped.")
                break

        # Call the on_training_end of the callbacks
        callback_logs = callbacks.on_training_end(epoch_data, self)
...
class StateSpaceRecurrentLayer(nn.Module, metaclass=abc.ABCMeta):
    ...
    def check_init_fcns(self):
        ...
        if self.init_input_ is None:
            self.init_input_ = lambda x: nn.init.uniform_(
                x, -1.0 / self.state_size, 1.0 / self.state_size
            )
        else:
            self._check_init_inplace(self.init_input_)

        if self.init_kernel_ is None:
            self.init_kernel_ = lambda x: nn.init.uniform_(
                x, -1.0 / self.state_size, 1.0 / self.state_size
            )
        else:
    ...
    def reset_parameters(self):
        ...
        for wname, weight in self.named_parameters():
            ngates = max(weight.shape[-1] // self.state_size, 1)
...
class StateSpaceGRU(StateSpaceRecurrentLayer):
    ...
    def compute_gamma(self) -> Tensor:
        """
        Compute the gamma_i value
        ...
        norms = self._get_norms(p=np.inf)
        gamma = 0.25*(1+norms["phi_r"])*norms["Wz_norm"] + norms["sigma_z"]*(norms["Wr_norm"]+0.25*norms["Ur_norm"]*norms["Wf_norm"])
        return gamma

class StateSpaceLTI(StateSpaceRecurrentLayer):
    ...
    def __init__(
        ...
        super(StateSpaceLTI, self).__init__(name)
        self.units = units
        self.in_features = in_features
        self.trainable = trainable
        self.io_delay = io_delay

        self.A = Parameter(torch.zeros(units, units), requires_grad=trainable)
        self.B = Parameter(torch.zeros(in_features, units), requires_grad=trainable)
        # self.reset_parameters()

    def extra_repr(self) -> str:
        return (
            f"units={self.units}, in_features={self.in_features}, trainable={self.trainable}"
        )
    ...
    def forward(self, u: Tensor, x0: Tensor = None) -> Tuple[Tensor, Tensor]:
        ...
        xp = torch.matmul(x0, self.A) + torch.matmul(u, self.B)  # (shape: (batch_size, n_units))
        y = x0 if self.io_delay else xp
...
class SoftPieceWiseRegularizer(nn.Module):
