...
class SequenceScaler:
    ...
    def __post_init__(self):
        if self.bias and not isinstance(self.bias, torch.Tensor):
            self.bias = torch.tensor(self.bias)
        if self.scale and not isinstance(self.scale, torch.Tensor):
...
class MeanSequenceScaler(SequenceScaler, EmpiricalScaler):
    ...
    def from_data(self, X: torch.Tensor):
        ...
        self.scale = (X - self.bias).abs().max(dim=dims) * (1.0 + self.rho) + self.eps
...
def tbptt(training: ExperimentData, 
    ...
    def to_list_tensors(experiments: ExperimentData) -> TensorExperimentData:
        ...
        """
        exps = [experiments] if not isinstance(experiments[0], Tuple) else experiments
